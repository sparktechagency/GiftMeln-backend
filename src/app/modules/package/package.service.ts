...
/**
 * update package into DB
 * @param id - Package ID to update
 * @param payload - Partial package data to update
...
const updatePackageIntoDB = async (id: string, payload: Partial<IPackage>) => {
  ...
  if ((payload.name || payload.description) && existingPackage.productId) {
    try {
      // Try updating the product metadata
      ...
      if (error.type === 'StripeInvalidRequestError') {
        // Fallback: Create a new product if the existing one cannot be found
        const newProduct = await stripe.products.create({
          name: payload.name || existingPackage.name,
          description: payload.description || existingPackage.description,
        });

        // Update the productId with the new product ID
        existingPackage.productId = newProduct.id;
        // Optionally update your database with the new productId
        await Package.findByIdAndUpdate(id, { productId: newProduct.id });
      } else {
        throw error; // If it's not the expected error, rethrow it
      }
    }
  }
  ...
  if (isPriceChanging) {
        ...
        line_items: [
          {
            price_data: {
              ...
              unit_amount:
                (payload?.price || existingPackage?.price || 0) * 100,
              recurring: {
                interval:
    ...
    // For PAID packages
      ...
      const newPrice = await stripe.prices.create({
        ...
        unit_amount: (payload?.price || existingPackage?.price || 0) * 100,
        currency: 'usd',
        recurring: {
